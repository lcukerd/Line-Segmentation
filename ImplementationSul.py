...
def performComputation(fileName):
    ...
    try:
        countSul = performPapvassiliouSegmentation(fileName);
    except:
        print (fileName);
        countSul = count;
    if (countSul <= count + 5 and countSul >= count - 5):
        if (debug == 'd'):
            print ("Correct: Got " + str(countSul) + " / " + str(count) + " for " + fileName);
        return 1;
    else:
        if (debug == 'd'):
            print ("Wrong: Got " + str(countSul) + " / " + str(count) + " for " + fileName);
        return 0;
...
for i in range(int (len(files)/cores) + (1 if len(files)%cores != 0 else 0)):
    ...
    for j in out:
        accAli += j;
    print ("Accuracy:" + str((i+1)*cores) + " Suleman: " + str(accAli / ((i+1)*cores if (i+1)*cores < len(files) else len(files)-1)));

print ("Accuracy: Suleman: " + str(accAli / len(files)));
