...
def performComputation(fileName):
    ...
    try:
        ...
        print (fileName);
        countPap = count;
    if (countPap <= count + 5 and countPap >= count - 5):
        if (debug == 'd'):
            print ("Correct: Got " + str(countPap) + " / " + str(count) + " for " + fileName);
        return 1;
    else:
        if (debug == 'd'):
            print ("Wrong: Got " + str(countPap) + " / " + str(count) + " for " + fileName);
        return 0;
...
for i in range(int (len(files)/cores) + (1 if len(files)%cores != 0 else 0)):
    ...
    for j in out:
        accAli += j;
    print ("Accuracy:" + str((i+1)*cores) + " Papvassiliou: " + str(accAli / ((i+1)*cores if (i+1)*cores < len(files) else len(files)-1)));

print ("Accuracy: Papvassiliou: " + str(accAli / len(files)));
